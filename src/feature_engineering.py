...
class FeatureEngineering():
    ...
    def train_test_split(self, df=None, seasons_train=[20162017, 20172018, 20182019], season_test=20192020):
        if not df:
            df = self.df
        df_train = df[(df['season'].isin(seasons_train)) & (df['gameType'].isin(['R']))]
        df_test = df[df['season'] == season_test]
        return df_train, df_test

    def adjustdata(self, df):
        train, test = self.train_test_split(df=df)
