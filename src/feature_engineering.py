...
class FeatureEngineering():
    ...
    def model_features(self, df):
        ...
        onehot_encoded_test = pd.get_dummies(x_test, columns=['last_event'], prefix='last_event')
        onehot_encoded_test['shot_type'] = pd.Categorical(onehot_encoded_test['shot_type'])
        onehot_encoded_test = pd.get_dummies(onehot_encoded_test, columns=['shot_type'], prefix='shot_type')
        x_test = onehot_encoded_test.fillna(0).astype(int)
